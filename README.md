...
│   ├── context_menu.py    # 目标网页右键菜单注入脚本（刷新/复制/粘贴/打开配置页）
...
- 在目标网页上有两个入口：右键菜单的「打开配置页」，以及右下角的可拖动齿轮图标
  （单击即进入）——两者都以遮罩方式打开配置页，不刷新页面、服务保持运行；
  错误页进入的配置页为整页切换。遮罩式配置页显示右上角「✕」按钮，点击关闭并返回
  来源页面；首次运行进入的配置页不显示该按钮
